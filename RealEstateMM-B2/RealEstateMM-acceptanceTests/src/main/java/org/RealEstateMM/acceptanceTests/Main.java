package org.RealEstateMM.acceptanceTests;
...
import org.RealEstateMM.steps.VendorConnectionSteps;
...
public class Main extends JUnitStories {

	private Format[] formats = new Format[] { CONSOLE };
	private StoryReporterBuilder reporterBuilder = new StoryReporterBuilder()
			.withCodeLocation(codeLocationFromClass(Main.class)).withFailureTrace(true)
			.withFailureTraceCompression(true).withDefaultFormats().withFormats(formats);
	private Embedder embedder = new Embedder();

	public static void main(String[] args) {
		new Main().embedder.runAsEmbeddables(Arrays.asList(Main.class.getCanonicalName()));
	}
	...
	public InjectableStepsFactory stepsFactory() {
		return new InstanceStepsFactory(configuration(), new VendorConnectionSteps());
	}
